...
function parseTimeString(timeStr: string | undefined, useMeridiem: boolean): TimeValue {
	if (!timeStr) return {}
...
function timeToMinutes(value: TimeValue, useMeridiem: boolean): number | undefined {
	if (value.hours === undefined) return undefined
...
function TimePickerColumn({
	...
	return (
		<ScrollArea
			...
			aria-label={ariaLabel}
		>
			<div ref={containerRef} className="flex flex-col py-2" role="listbox" aria-label={ariaLabel}>
				{options.map((option) => {
					...
					return (
						...
						>
							...
							{isSelected && <CheckIcon className="absolute right-1 size-3.5 shrink-0" />}
						</button>
					)
				})}
			</div>
...
function TimePickerContent({
	...
	const minuteOptions = React.useMemo(() => generateMinuteOptions(minuteStep), [minuteStep])

	const secondOptions = React.useMemo(() => generateSecondOptions(secondStep), [secondStep])
	...
	if (showHours) {
		visibleColumns.push(
			<TimePickerColumn
				...
				onValueChange={(v) => onValueChange({ ...value, hours: parseInt(v, 10) })}
	...
	if (showMinutes) {
		visibleColumns.push(
			<TimePickerColumn
				...
				onValueChange={(v) => onValueChange({ ...value, minutes: parseInt(v, 10) })}
	...
	if (showSeconds) {
		visibleColumns.push(
			<TimePickerColumn
				...
				onValueChange={(v) => onValueChange({ ...value, seconds: parseInt(v, 10) })}
	...
	if (useMeridiem) {
		visibleColumns.push(
			<TimePickerColumn
				...
				onValueChange={(v) => onValueChange({ ...value, meridiem: v as "AM" | "PM" })}
	...
	return (
		<div className={cn("flex", className)} role="group" aria-label="Time picker">
...
function TimePicker({
	...
	const handleValueChange = React.useCallback(
		(newValue: TimeValue) => {
			...
			const formatted = formatTimeValue(newValue, useMeridiem, showMinutes, showSeconds)
	...
	return (
		<Popover open={open} onOpenChange={setOpen}>
			...
			<PopoverContent className={cn("w-auto p-3", contentClassName)} align="start">
				<TimePickerContent
					value={currentValue}
